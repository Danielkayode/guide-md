...
const SupportedLanguage = z.enum([
  ...
  "java", "kotlin", "swift", "cpp", "c", "csharp",
  "ruby", "php", "scala", "haskell", "elixir", "zig",
]);
...
const CodeStyleSchema = z.object({
  ...
  indentation: z
    .union([z.literal("tabs"), z.string().regex(/^\d+ spaces$/, 'Must be "tabs" or e.g. "2 spaces"')])
    .default("2 spaces"),
...
const GuardrailsSchema = z.object({
  ...
  cite_sources: z
    .boolean()
    .default(false)
    .describe("AI should include inline comments citing documentation when using unfamiliar APIs"),

  scope_creep_prevention: z
    .boolean()
    .default(true)
    .describe("AI must only modify files/functions explicitly referenced in the prompt"),
  ...
  max_response_scope: ScopeEnum
    .default("function")
    .describe("Max unit of code AI should generate unprompted"),
});
...
const TestingSchema = z.object({
  ...
  test_alongside_code: z
    .boolean()
    .default(false)
    .describe("If true, AI should generate tests in the same response as implementation"),
});
...
const ContextSchema = z.object({
  ...
  architecture_pattern: z
    .enum(["mvc", "clean", "hexagonal", "layered", "microservices", "monolith", "serverless", "event-driven"])
    .optional(),
...
export const GuideMdSchema = z.object({
  ...
  runtime: z.string().optional().describe('e.g. "node@22", "bun@1.1", "python@3.12"'),
  ...
  error_protocol: ErrorProtocol
    .describe("How AI should handle and surface errors in generated code"),
...
// ─── Type Export ─────────────────────────────────────────────────────────────
/** @typedef {import('zod').infer<typeof GuideMdSchema>} GuideMdFrontmatter */
